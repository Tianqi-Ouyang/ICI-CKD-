...
# Apply mean(SD) only to variables that passed the skewness check above
tbl1_stat_spec <- lapply(normal_vars, function(v) {
  as.formula(paste0("`", v, "` ~ \"{mean} ({sd})\""))
})
...
final_master_ckd %>%
  ...
  tbl_summary(missing_text = "Missing", by = aki_before_ckd)
```
