...
# Curated labels from the analytic cohort (used for Included patients)
curated_labels <- final_master %>%
  transmute(
    EMPI,
    cancer_group_curated = as.character(cancer_group),
    ICI_Class_curated    = as.character(ICI_Class)
  )

compare_df <- raw_starts %>%
  merge(dem_supp,        by = "EMPI", all.x = TRUE) %>%
  merge(curated_labels,  by = "EMPI", all.x = TRUE) %>%
  mutate(
    ...
    # Final labels: curated for Included, raw mapping for Excluded
    #   → ensures §12.1 Included column matches §3.1 exactly
    cancer_group_supp = coalesce(cancer_group_curated, cancer_group_raw),
    ICI_Class_supp    = coalesce(ICI_Class_curated,    ICI_Class_raw),
